...
export default function ResultsPage({ rows, evidenceCounts }: { rows: ProgramRow[]; evidenceCounts: Record<string, number> }) {
  ...
  <div className="container py-7 pb-14">{rows.length === 0 ? <div className="space-y-5"><div className="ledger-card surface-dots flex min-h-72 flex-col items-center justify-center p-8 text-center"><Database className="h-7 w-7 text-primary" /><h3 className="mt-4 text-lg font-extrabold">No programmes researched yet</h3><p className="mt-2 max-w-md text-sm leading-6 text-muted-foreground">Queue a research pass below — finished runs appear here automatically.</p></div><ResearchTrigger /></div> : <div className="space-y-5"><ResearchTrigger /><div>{filters.length === 0 ? <ProgramTable rows={sorted} /> : <EmptyResults onReset={reset} />}</div></div>}</div>
  </div>;
}
