...
export default function ComparePage() {
  ...
  const rows: Array<{ label: string; get: (program: ProgramRow) => React.ReactNode; best?: (program: ProgramRow) => boolean }> = [
    ...
    { label: 'Match', get: (p) => <MatchPill match={p.matchClass} />, best: (p) => p.matchClass === 'HIGH' },
    { label: 'Score', get: (p) => <span className="tabular text-xl font-extrabold text-primary">{p.score}</span>, best: (p) => p.score === Math.max(...selected.map((candidate) => candidate.score)) },
  ];
