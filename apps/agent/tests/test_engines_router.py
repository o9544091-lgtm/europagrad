...
from html_fixtures import JS_SHELL_HTML, PROGRAM_HTML
...
def ok(engine: str, html: str = PROGRAM_HTML) -> FetchResult:
    return FetchResult(url="https://x.edu/p", status=200, engine=engine, html=html, text_chars=900, links=[])


def shell(engine: str) -> FetchResult:
    return FetchResult(url="https://x.edu/p", status=200, engine=engine, html=JS_SHELL_HTML, text_chars=30, links=[])
